...
Before the first message to a node in a conversation, list that node's sessions with `msg sessions` and ask the operator whether to use an existing session there or a new intercom session (`msg send <node> --new <runtime>`, passing the operator's answer as `--directive`). Never choose on your own; later messages of the same conversation go to the session the operator chose.

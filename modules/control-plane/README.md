...
node modules/control-plane/node/cli.mts msg send <node> --new claude|codex [--cwd <dir>] [--directive <text>] [--from <session>] [--wait <seconds>] [--] <text...>
...
- `msg send <node> --new claude|codex [--cwd <dir>] -- <text>` resolves the node by id or name from the directory and writes a delegated task request (see [Delegated tasks](#delegated-tasks)) with `requirements.node` set to that node, the runtime and `--cwd` as requirements, the label `intercom: <own runtime>@<own node name>` as `label` and `title`, and the text as the task text. The own runtime is `codex` for a `--from` that names a recorded Codex session, otherwise `claude` when `CLAUDE_CODE_SESSION_ID` is set. `--directive` carries the operator's answer; without it the directive is `The operator chose a new intercom session for this conversation.`

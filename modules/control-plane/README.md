...
- **Continue.** For an ended run with a known thread: `codex exec resume --json -c sandbox_mode="<sandbox>" -c sandbox_workspace_write.writable_roots=["<outbox>"] -o <last message file> <thread_id> -` in the task's working directory, with the follow-up on stdin and a new deadline. `codex exec resume` has no `-C`, `--sandbox` or `--add-dir` flag.
...
- **Windows.** A `codex.cmd` shim is refused; the node needs the native `codex` executable on `PATH`.

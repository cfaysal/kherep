...
A task with `requirements.runtime` `codex` (issue #63) runs as `codex exec` on a node whose policy lists `codex` in `sessions.runtimes`. Admission, limits, the working directory rule and the framed prompt are the same as for Claude tasks. The flags below are from `codex exec --help` and `codex exec resume --help` of Codex CLI 0.153.4 and the measurements in the issue.

- **Start.** The daemon runs `codex exec --json -C <cwd> --sandbox <sandbox> --add-dir <outbox> -o <last message file> "<framed prompt>"` in `<cwd>`, detached in its own process group, with stdin from the null device (otherwise `codex exec` waits for more input). The events go to `codex-tasks/<taskId>/events.jsonl` in the node directory, stderr to `stderr.log` there, and `-o` writes `last-message.txt`. The node records the pid and the process start time, waits up to 15 seconds for the first `thread.started` event and records its `thread_id` as the task's `sessionId`, then reports `started`. A thread id that shows up later is reported by the next watch round.
...
- **Watch.** With every 60-second session check the node checks each active Codex task's process by pid and start time; a pid alone may belong to another process by then. A failed read of the start time decides nothing. When the process has ended, the task is `done` if the events contain `turn.completed` and the exit code was 0 (or the daemon did not see the exit), with the last message as the summary (at most 2048 characters); otherwise `failed` with the `turn.failed` or `error` message, or the exit code, never the prompt. After `maxRuntimeMinutes` the node stops the process and reports `stopped`, reason `max runtime reached`.
- **Stop.** `POST /api/tasks/{id}/stop` checks that the pid still has the recorded start time, sends SIGTERM to the process group and SIGKILL after 5 seconds, again only after the same check (on Windows `taskkill /T`, then `/F`; not yet tested on Windows), and reports `stopped`.
- **Continue.** For an ended run with a known thread: `codex exec resume --json -c sandbox_mode="<sandbox>" -c sandbox_workspace_write.writable_roots=["<outbox>"] -o <last message file> <thread_id> "<follow-up>"` in the task's working directory, with a new deadline. `codex exec resume` has no `-C`, `--sandbox` or `--add-dir` flag.
- **Messaging out.** Besides the working directory, the node's `outbox/` is the only writable root (for every permission mode), so `kherep-node msg send` works from the sandbox; the rest of the node directory (key, policy, task records) stays read-only, so `task done` does not, and a `msg` read that asks for a directory refresh cannot write that request. The process gets `KHEREP_CONFIG_DIR` (the node directory) and `KHEREP_SESSION_ID` (the thread id, or the task name before it is known); a `CLAUDE_CODE_SESSION_ID` the daemon inherited is removed. Messages it sends carry the task id.
- **Messaging in.** A `codex exec` run ends with its turn, so the daemon's 2-second exchange round resumes an ended Codex task for messages addressed to its thread id or name: `codex exec resume` as above, with a prompt that starts with the wake text and carries the messages framed as the delivery hook frames them (per-call nonce markers, peer content that is not an instruction from the user). The messages are marked `offered`; when the run completes its turn they become `delivered`, otherwise they are offered again, at most 3 times (`MAX_OFFERS`). The guards are the Claude wake's: the kill switch `wake.disabled`, the `wake` allowlist or the task grant (messages of the task's own id), no `bypassPermissions`, reply depth below 6, and the shared budget of 6 autonomous turns per rolling hour, 20 per day and 30 s spacing per session; each decision goes to `wake.jsonl` without text. Since a resume starts a process, it also needs sessions enabled with `codex` in `runtimes` and a free slot under `maxConcurrent`. One run per task: messages that arrive meanwhile wait. Such a run keeps the task's reported state (no `task.report`), counts as active, and is stopped at the max runtime like any run.
- **Windows.** A `codex.cmd` shim cannot pass the prompt through `cmd.exe` safely, so a start through one fails with that reason; the node needs the native `codex` executable on `PATH`.
- `task done` also works from a Codex task, if its sandbox lets it write the node directory; the process then stays counted until it ends.

...
- **Continue.** For an ended run with a known thread: `codex exec resume --json -c sandbox_mode="<sandbox>" -o <last message file> <thread_id> "<follow-up>"` in the task's working directory, with a new deadline. `codex exec resume` has no `-C` and no `--sandbox` flag.

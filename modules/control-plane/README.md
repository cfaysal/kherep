...
`session.list` and the `sessions.snapshot` frame report the agent sessions running on the node. A session carries `sessionId`, `runtime`, `state` and optional `startedAt` (ISO 8601), `name` (at most 128 chars), `cwd` (at most 512), `kind` (at most 32) and `label`. `name`, `cwd` and `kind` were added in Phase 2, `label` with issue #74; a node that omits them stays valid, and the Registry stores them as nullable columns that it adds to an existing `sessions` table on start. `label` is the display name of a task session that was started with one (letters, digits, space, `:`, `@`, `-`, `_` and `.`, at most 64 characters, no leading or trailing space), for example `intercom: claude@sekhmet`; the node adds it from its task record, found by session id or by the `task-<8>` name.
...
- Codex: Codex documents no session listing, so the node lists the sessions its delivery hook recorded in `codex-sessions/` and saw within the last 12 hours (`CODEX_ACTIVE_MS`), with `runtime` and `kind` `codex`, `state` `active` and the name `codex-<last 8 characters of the id>` (Codex thread ids are UUIDv7, whose first characters are a timestamp, so sessions started within about a minute shared the earlier `codex-<first 8>` name; that name is still accepted as an address where exactly one live session has it, and a name two live sessions share addresses neither) (see [Codex sessions](#codex-sessions)).
- Codex tasks: the node also lists each Codex task it started, from `tasks/`, once its thread id is known and while it is active or was updated within the last 12 hours: `sessionId` the thread id, `runtime` `codex`, `kind` `codex-task`, the task's name (`task-<8>`), `state` `running` or `idle`. A hook record for the same thread is listed once, as the task (see [Codex tasks](#codex-tasks)).
...
node modules/control-plane/node/cli.mts msg send <node> --new claude|codex --directive <the operator's answer, verbatim> [--cwd <dir>] [--from <session>] [--wait <seconds>] [--] <text...>
node modules/control-plane/node/cli.mts msg inbox [--all]
node modules/control-plane/node/cli.mts msg status <messageId>
```

- `msg sessions` prints every node of the directory with its sessions (the label if there is one, otherwise the name; id, state, runtime, cwd) and marks this session with `*`. A missing `directory.json` is an error that says the daemon may not be running, never an empty list; a directory older than 3 minutes is printed with a warning.
- `msg send` resolves the node by id or name, then the session on that node by id, name or label. An unknown or ambiguous reference fails and lists the candidates. The address carries the resolved session id (names are for display and stay valid only until a rename); the target node's policy matches a rule by that id or by the session's current name in its `sessions.json`, and `*` as before. A message addressed by name, from an older sender or the operator API, still works as before. `--reply-to` sets `inReplyTo` and sends to the sender of that inbox message unless `--to` is given. The message is written to the outbox and its id printed; `--wait` waits for `accepted` or a refusal and exits non-zero unless the message was accepted. Put `--` before text that starts with `--`.
- The sender session is `--from` (a Codex session id recorded in `codex-sessions/` becomes that session's `codex-...` name), otherwise the name `sessions.json` records for `CLAUDE_CODE_SESSION_ID` (or, without it, the runtime-neutral `KHEREP_SESSION_ID` the node sets for Codex task sessions), otherwise that id. Claude Code sets the variable in Bash and PowerShell tool, hook and stdio MCP subprocesses ([environment variables](https://code.claude.com/docs/en/env-vars)). Without either, `msg send` fails.
...
- Codex sessions get no listener. The daemon wakes an idle interactive Codex session with `codex queue` instead (see [Waking Codex sessions](#waking-codex-sessions)); without that, their messages arrive with the next prompt.
- The Codex `Stop` continuation (`decision` `block`, see [Codex sessions](#codex-sessions)) goes through the same check as the Claude one: never in `bypassPermissions` (from the hook input's `permission_mode`), and within the shared budget.
...
An idle interactive Codex session (the Codex TUI or app) runs no hook until its next prompt, so the daemon's 2-second exchange round wakes it for peer messages (issue #66). Measured with Codex CLI 0.153.4: `codex queue --thread <id> --message <text>` queued a message into an idle app session, which ran a turn by itself within 15 seconds; the text becomes a user turn.

- The node runs `codex queue --thread <session id> --message "Kherep: <n> new message(s) from other agent sessions arrived. They are delivered in this turn."`: a fixed pointer with the count only, never peer text or sender names, as the Claude wake does. The woken turn's `UserPromptSubmit` delivery hook then offers the framed messages (offer and confirm unchanged). If that hook does not run (hook trust missing), the messages stay `accepted`; the queue never carries them.
- Candidates are the sessions in `codex-sessions/` seen within 12 hours, except the threads of Codex tasks, which are resumed instead (see [Codex tasks](#codex-tasks)); the task grant therefore does not apply.
- Guards as for the Claude wake: the kill switch `wake.disabled`, a `wake` section that lists the session by its full id (or `"*"`; a `codex-<8>` name never authorizes, since names can be shared), never a session whose recorded permission mode is `bypassPermissions`, reply depth below 6, and the shared budget of 6 autonomous turns per rolling hour, 20 per day and 30 s spacing per session. A session whose permission mode the hook never recorded is woken only when the `wake` list names it; `"*"` is not enough. Each decision goes to `wake.jsonl` without text (`wake`, `not-allowlisted`, `disabled`, `permission-mode`, `permission-mode-unknown`, `depth-limit`, `budget`, `queue-failed`).
- One wake at a time: each message causes at most one queue, and a session gets no further queue while a queued message is still `accepted` and younger than 10 minutes. A queued message not offered by then waits for the next prompt instead of being queued again, and a failed queue is not retried; its reason (codex's last stderr line, redacted) goes to the daemon log.
- `codex queue` gets only `--thread` and `--message`; the node refuses `--dangerously-*`, `--approve-for-me`, `--add-dir`, `--sandbox`/`-s` and `-c`/`--config`. Messages addressed to a name that more than one live session shares wake neither session (`ambiguous-name`). It runs without a shell, on Windows through the npm launcher as for Codex tasks, and never inside the exchange round: runs go to their own serial lane, at most one per session. A run settles when its process exits; after 30 seconds the node kills its whole process tree (the process group, or `taskkill /T /F` on Windows, where the launcher's `codex.exe` would otherwise keep the pipe open).
- Not measured yet: whether `codex queue` works when no app server or TUI owns the thread, and whether the woken turn runs the Kherep hooks.

## Tasks

A task is an instruction that a node runs as a Claude Code background session (item 5, operator decisions of 2026-09-25) or, with runtime `codex`, as a `codex exec` run (issue #63, see [Codex tasks](#codex-tasks)). A node runs Codex tasks only when its policy lists `codex` in `sessions.runtimes`; otherwise it refuses them with the reason `runtime codex is not supported on this node yet`.
...
- **Node selection.** With `requirements.node` (a node id, issue #74) the Worker takes exactly that node when it is online, not revoked and fits every other requirement below, and otherwise refuses with the reason (`unknown node`, `is not online`, `does not advertise ...`, `runs <os>, not <os>`, `has no <runtime> CLI runtime`); it never falls back to another node. Without it the Worker takes online, non-revoked nodes that advertise `sessions.v1`, list the runtime (`claude` or `codex`, default `claude`) as a CLI runtime and match `requirements.os` (the node's `os` fact, for example `win32`, `darwin`, `linux`) and every `requirements.capabilities` entry; among them the node with the fewest active tasks, by name on a tie. Without one it answers 409 with the reason and audits `task.refuse`. It stores the task as `dispatched` and queues `session.start` with `taskId`, `runtime`, `name` (`task-<first 8 of taskId>`), `prompt` (the task text), `permissionMode` (default `auto`; `auto`, `default` or `acceptEdits`, never `bypassPermissions`), `requirements.cwd` as `cwd` and, when the task has one, `label`. `POST /api/tasks` takes an optional `label`; the Registry keeps it in the `tasks` table (a column added on start) and the node in `tasks/<taskId>.json`.
- **Start.** The node checks its `sessions` policy, the runtime, the permission mode, its limits and the working directory, then runs `claude --bg --name task-<8> --permission-mode <mode> "<framed prompt>"` in that directory (also for a task with a label: the node maps the session by that name until it knows the short id, and the Worker and the node recognise a task session by it, while a label is shared by every intercom session from one sender; the label is shown only in the directory), with the same executable lookup as the session listing. The prompt is one argument of a process started without a shell. A Windows npm install puts a `claude.cmd` shim on `PATH`; when the native `claude.exe` the package ships sits next to it (`node_modules/@anthropic-ai/claude-code/bin/claude.exe`), the node runs that executable directly. A shim without it needs `cmd.exe`, which cannot carry arbitrary text safely, so a start or continue through such a shim fails with that reason. `--bg` prints `backgrounded · <short id> · <name>` (the [agent view documentation](https://code.claude.com/docs/en/agent-view), fetched 2026-09-25); the node records the short id and maps the full session id from `claude agents --json --all` (by that short id, else by name) into `tasks/<taskId>.json` with `name`, `cwd`, `permissionMode`, `startedAt`, `deadline` and `state`, and reports `started`. A refusal or a CLI error, for example a CLI that is not signed in, reports `failed` with the CLI's own message (at most 256 characters), never the command line.
...
- **Limits.** Claude and Codex tasks share the node's limits: `maxConcurrent` and `maxStartsPerDay` count both.
...
- Prerequisite: the Claude CLI (and, for Codex tasks, the Codex CLI) must be installed and signed in for the account the daemon runs as; task sessions use that account's stored credentials.
...
node modules/control-plane/node/cli.mts task new --title <title> --directive "<the operator's instruction, verbatim>" [--runtime claude|codex] [--os <os>] [--cwd <dir>] [--capability <name>]... -- <task text>
```
...
A task with `requirements.runtime` `codex` (issue #63) runs as `codex exec` on a node whose policy lists `codex` in `sessions.runtimes`. Admission, limits and the working directory rule are the same as for Claude tasks. The flags below are from `codex exec --help` and `codex exec resume --help` of Codex CLI 0.153.4 and the measurements in the issue.

- **Start.** The daemon runs `codex exec --json --skip-git-repo-check -C <cwd> --sandbox <sandbox> --add-dir <outbox> -o <last message file> -` in `<cwd>`, detached in its own process group. The framed prompt goes in on stdin (`-`: "instructions are read from stdin"), and stdin is closed right after it, so `codex exec` does not wait for more input and the task text never shows in the process list. The events go to `codex-tasks/<taskId>/events.jsonl` in the node directory, stderr to `stderr.log` there, and `-o` writes `last-message.txt`. The node records the pid and the process start time, waits up to 15 seconds for the first `thread.started` event and records its `thread_id` as the task's `sessionId`, then reports `started`. A thread id that shows up later is reported by the next watch round.
- **Git check.** `--skip-git-repo-check` skips only Codex's check that the working directory is a trusted Git repository, not the sandbox; the node already confines the working directory to its workspace roots, which need not be repositories. Without it such a task fails at once with `Not inside a trusted directory and --skip-git-repo-check was not specified.` (measured on macOS).
- **Framed prompt.** As for Claude tasks, except the closing instruction: a Codex task ends its turn with a short summary, which is reported as the task's result, instead of running `task done`.
- **Sandbox.** `codex exec` cannot ask for approval, so the permission mode maps to a sandbox: `auto` and `acceptEdits` to `workspace-write`, `default` to `read-only`. The node never passes `--dangerously-bypass-approvals-and-sandbox`, `--dangerously-bypass-hook-trust` or `--approve-for-me`, and refuses a command line that contains one.
- **Watch.** With every 60-second session check the node checks each active Codex task's process: while the daemon still holds the child it started (its pid cannot be reused before it is reaped), otherwise by pid and recorded start time. A start time that could not be read at the start is read again while the child is held. A failed read decides nothing. When the process has ended, the task is `done` if the events contain `turn.completed` and the exit code was 0 (or the daemon did not see the exit), with the last message as the summary (at most 2048 characters); otherwise `failed` with the `turn.failed` or `error` message, or the exit code followed by codex's last non-empty stderr line (API keys, bearer tokens, JWTs, URL user info and URL query strings redacted, control characters removed, at most 200 characters, dropped when it quotes the framed prompt, which names Kherep in any case), never the prompt. A run without start time, held child or recorded exit (after a daemon restart) is reported `failed` with the reason `process identity unknown` instead of keeping its slot; its process, if it still runs, is not stopped. After `maxRuntimeMinutes` the node stops the process and reports `stopped`, reason `max runtime reached`.
- **Stop.** `POST /api/tasks/{id}/stop` signals the process group only if the daemon holds the child or the pid still has the recorded start time: SIGTERM, then SIGKILL after 5 seconds after the same check (on Windows `taskkill /T`, then `/F`; not yet tested on Windows), and reports `stopped`. A task that reported `done`, and a run started for peer messages, keep their reported state and send no report.
- **Continue.** For an ended run with a known thread: `codex exec resume --json --skip-git-repo-check -c sandbox_mode="<sandbox>" -c sandbox_workspace_write.writable_roots=["<outbox>"] -o <last message file> <thread_id> -` in the task's working directory, with the follow-up on stdin and a new deadline. The working directory is checked again first, as at the start, so a directory swapped for a link out of the workspace roots is refused; the thread id must be a plain id (it cannot start with `-`). `codex exec resume` has no `-C`, `--sandbox` or `--add-dir` flag.
- **Messaging out.** Besides the working directory, the node's `outbox/` is the only extra writable root, so `kherep-node msg send` works from a `workspace-write` sandbox (`auto`, `acceptEdits`); the rest of the node directory (key, policy, task records) stays read-only, and a `msg` read that asks for a directory refresh cannot write that request. The [configuration reference](https://developers.openai.com/codex/config-reference) documents extra writable roots only "when `sandbox_mode = "workspace-write"`", so a Codex task in permission mode `default` (`read-only`) most likely cannot send messages; the node passes the root anyway, and whether Codex honours it there is not measured yet. The process gets `KHEREP_CONFIG_DIR` (the node directory) and `KHEREP_SESSION_ID` (the thread id, or the task name before it is known); a `CLAUDE_CODE_SESSION_ID` the daemon inherited is removed. Messages it sends carry the task id.
- **Messaging in.** A `codex exec` run ends with its turn, so the daemon's 2-second exchange round resumes an ended Codex task for messages addressed to its thread id or name: `codex exec resume` as above, in the task's working directory, checked again as for a continue, with a prompt (on stdin) that starts with the wake text and carries the messages framed as the delivery hook frames them (per-call nonce markers, peer content that is not an instruction from the user). The messages are marked `offered`; when the run completes its turn they become `delivered`, otherwise they are offered again, at most 3 times (`MAX_OFFERS`). The guards are the Claude wake's: the kill switch `wake.disabled`, the `wake` allowlist or the task grant (messages of the task's own id), no `bypassPermissions`, reply depth below 6, and the shared budget of 6 autonomous turns per rolling hour, 20 per day and 30 s spacing per session; each decision goes to `wake.jsonl` without text. Since a resume starts a process, it also needs sessions enabled with `codex` in `runtimes` and a free slot under `maxConcurrent`. One run per task: messages that arrive meanwhile wait. Such a run keeps the task's reported state (no `task.report`), counts as active, and is stopped at the max runtime like any run.
- **Windows.** An npm install puts a `codex.cmd` shim on `PATH`, and `cmd.exe` cannot carry arbitrary text safely. The shim only runs `node "%dp0%\node_modules\@openai\codex\bin\codex.js" %*` (measured on a Windows node), so the node runs that launcher itself with its own Node executable and without a shell. The launcher's setup is kept: it finds the native `codex.exe` of the platform package (`@openai/codex-win32-<arch>`) and sets `CODEX_MANAGED_PACKAGE_ROOT` and `CODEX_MANAGED_BY_NPM` (read from `bin/codex.js` of `@openai/codex` 0.152.1). A shim without the launcher next to it, and any other `.cmd` or `.bat`, is refused. The process the node holds is then the launcher's Node, and codex runs as its child: stop and the deadline end the whole tree (`taskkill /T`, then `/T /F`; the process group on macOS and Linux).
- **Windows: not cleared for use.** On Windows the Codex sandbox (`workspace-write`, `--add-dir`, `writable_roots`) is not measured. Do not add `codex` to a Windows node's `sessions.runtimes` until a measured check passes: a Codex task that tries to write outside its working directory and the outbox must be denied. Known stop limits: the first step, `taskkill /T` without `/F`, usually does nothing to processes without a window, so the effective stop is `taskkill /T /F` after 5 seconds; and `taskkill /T` walks the tree by parent process id, which Windows does not clear when a parent ends, so it may include an unrelated process that reuses a stale parent id.
...
- `msg send <node> --new claude|codex --directive "<the operator's answer>" [--cwd <dir>] -- <text>` resolves the node by id or name from the directory and writes a delegated task request (see [Delegated tasks](#delegated-tasks)) with `requirements.node` set to that node, the runtime and `--cwd` as requirements, the label `intercom: <own runtime>@<own node name>` as `label` and `title`, and the text as the task text. The own runtime is `codex` for a `--from` that names a recorded Codex session, otherwise `claude` when `CLAUDE_CODE_SESSION_ID` is set. `--directive` is required and quotes the operator's answer to the question existing session or new intercom session verbatim; it becomes the task's directive. There is no default, because a default would claim an operator choice nobody made: without `--directive` the CLI exits 1 with `--new needs --directive with the operator's answer` and writes nothing.
- It then waits (default 30 seconds, `--wait`) for the Worker's answer that the daemon records, prints the task id and exits 0; a refusal prints `refused: <reason>` and exits 1, as does no answer within the wait (the request stays pending; `task show <requestId>` shows it later). The checks of `task new` apply first and fail at once: no chains, `sessions.delegate.request` on this node, no active task on this node. The target needs `sessions.delegate.accept`, sessions enabled and its limits; the Worker refuses with the reason when it does not qualify.
- The target starts the session as a background task named `task-<8>` with the label recorded; `msg sessions` shows it under its label. The framed prompt adds that it is an intercom session for a conversation with `<nodeId>/<session>` and how to answer: `kherep-node msg send <nodeId>/<session> -- "<answer>"`. Later messages of the conversation are ordinary `msg send` messages to that session.
- Needs a Worker that knows `requirements.node` and `label`; an older Worker refuses such a request as an invalid body, and an older node refuses a `session.start` with a `label`.
...
- **Tasks.** Only the operator API and a delegated request that passed the checks of both nodes and the Worker queue a session command; a node runs it only with sessions enabled in its own policy, inside its workspace roots, within its limits, never in `bypassPermissions`, and Codex tasks only when its policy lists `codex`, in a Codex sandbox, never with a bypass flag. The task text stays out of the audit table; see [Tasks](#tasks).
...
| `POST /api/tasks` | Body `{"title": "...", "text": "...", "requirements": {"runtime": "claude", "os": "win32", "capabilities": [], "cwd": "D:/work/repo"}, "permissionMode": "auto"}` (`requirements` and its fields and `permissionMode` optional); 201 with `taskId`, `nodeId`, `state`; 400 for an unknown runtime or a disallowed mode; 409 with the reason when no node fits |

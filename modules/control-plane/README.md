...
- Guards as for the Claude wake: the kill switch `wake.disabled`, a `wake` section that lists the session (id or `codex-<8>` name, or `"*"`), never a session whose recorded permission mode is `bypassPermissions`, reply depth below 6, and the shared budget of 6 autonomous turns per rolling hour, 20 per day and 30 s spacing per session. A session whose permission mode the hook never recorded is woken only when the `wake` list names it; `"*"` is not enough. Each decision goes to `wake.jsonl` without text (`wake`, `not-allowlisted`, `disabled`, `permission-mode`, `permission-mode-unknown`, `depth-limit`, `budget`, `queue-failed`).
...
- `codex queue` gets only `--thread` and `--message`; the node refuses `--dangerously-*`, `--approve-for-me`, `--add-dir`, `--sandbox`/`-s` and `-c`/`--config`. It runs without a shell, on Windows through the npm launcher as for Codex tasks, with a 30-second timeout.

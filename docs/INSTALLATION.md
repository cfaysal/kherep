...
The installer also wires the control-plane delivery hook, `modules/control-plane/node/deliver-hook.mts`, into the `UserPromptSubmit` and `Stop` hooks of `settings.json`. It runs from the checkout you install from, because it imports the modules next to it, so moving or deleting that checkout breaks the hook until you install again from the new location. The hook hands messages that other agent sessions sent through the Kherep control plane to their Claude Code session. It reads local files only, and on a machine without an enrolled control-plane node it finds no inbox and exits without output, so it is inert until you enroll one (see [the control plane](../modules/control-plane/README.md#delivery-hook)). It reads the node's default per-user config directory; when the daemon runs with `KHEREP_CONFIG_DIR`, set the same value in the `env` block of `settings.json`. The same hook also runs for `StopFailure`, so messages offered in a turn that ended with an API error are offered again. A second entry on `UserPromptSubmit` and on `Stop` runs `modules/control-plane/node/wake-hook.mts --timeout 86400` with `asyncRewake` and the same 24-hour `timeout`: it waits in the background for peer messages and wakes an idle session when one arrives. Arming at `UserPromptSubmit` too keeps a listener after a turn that ended without `Stop`. Waking stays off until the node's `policy.json` opts in with a section such as `"wake": { "enabled": true, "sessions": ["review"] }`, listing session ids or names (`"*"` for all, written explicitly); a missing or malformed section wakes nothing. Wakes and turns the delivery hook continues at `Stop` share a budget of 6 per rolling hour, 20 per rolling day and 30 s between two per session; a session in permission mode `bypassPermissions` is neither woken nor continued, and its messages wait for the next prompt. A listener whose Claude Code process has exited ends at its next poll; a `wake.disabled` file in the node directory switches waking off. Details: [listening while idle](../modules/control-plane/README.md#listening-while-idle). An upgrade replaces every existing entry that runs one of these control-plane scripts on the same event with the managed one, so a changed timeout never leaves two listeners; your other hooks stay as they are. To turn it off, remove these entries from `settings.json`; the next installation adds them back, and `drift-check.sh` reports the difference until then.

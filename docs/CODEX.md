...
Windows uses separate acceptance and observation Stop commands. macOS uses one combined Stop command that applies both checks; the same trusted Maestro publication rule applies on both hosts.

The normal Codex projection installs `codex-obs`, rendered from the Codex-only worker definition `codex/agents/codex-obs.md`, one turn-completion Stop hook and the minimal ten-file service-account Confluence graph in `<workspace>/tools`. The graph contains the Codex `atl-confluence.mts` broker, the shared `atlassian-credentials.mts` parser and the eight `confluence-*` modules for contract, content, session, related-page search, semantics, neighbours, neighbour CLI and runtime labels.

The Stop group runs the acceptance gate first and then requests exactly one observation continuation. The request tells the Maestro to dispatch `codex-obs` with `fork_turns: "none"` and to provide only the completed turn, relevant tool evidence and compact task state. In Codex, that worker performs no configuration or broker I/O and returns one strict JSON candidate envelope. Its projected role uses `sandbox_mode = "read-only"`, so it cannot write files; restricted subagent execution also does not grant it the Maestro's escalated network authority. Codex has no supported finer per-role tool-deny field in this projection, so the enforceable boundary is the read-only sandbox plus restricted worker execution, backed by the candidate-only instruction that forbids configuration and broker calls. Each candidate contains exactly `title`, `bodyStorage`, `evidence`, `labels` and `placement`; placement contains only `project` and `app`, while labels contain only `type-observation`, `evidence-<value>` and `status-author-model`. The trusted Maestro main thread validates it and adds session or runtime details during publication. An empty `observations` array performs zero writes; for a nonempty array, the main thread checks publication authority and runs related-page search, create/readback and stitch through the Codex broker projected by that installation. The installer renders the concrete `<selected-workspace>/tools/atl-confluence.mts` path into the installed hook as a PowerShell single-quoted literal, doubling embedded single quotes so spaces and `$()` remain literal; the installed artifact contains no placeholder or default-checkout fallback. The continued turn has `stop_hook_active=true`, so the hook does not request another pass.

This minimal graph does not enable the broader Atlassian surfaces. `KHEREP_INSTALL_ATLASSIAN_TOOLS=1` is the explicit opt-in for the Codex Jira graph, the Claude Jira and Confluence broker copies, and Rovo registration. The Claude bootstrap installer places both Confluence brokers by default, because it installs `claude-obs`. MPAC tooling remains separate and optional. Codex and Claude brokers continue to use separate service-account credential bindings.
...
Review and trust the exact hook definitions using the host's supported hook controls. Feature and administrator policies can prevent execution; installation does not override them. Inspect discovery and execution at the actual Desktop host, then verify the observation Stop pass as described above. Desktop hook support is UNKNOWN until this measurement. CLI discovery or a prepared output receipt alone does not prove delivery. See the [official Codex hook contract](https://developers.openai.com/codex/hooks).

For turn-completion observations, keep the following evidence separate:

On macOS, inspect `codex-observation-stop.mts` as the observation Stop command; on Windows, inspect `codex-observation-turn-completion.mts`.

1. **Configured target and readback:** confirm the installed `codex-obs` role, the platform's observation Stop entry, the ten required files in `<workspace>/tools` and the canonical JSON fields. Run `node <workspace>/tools/atl-confluence.mts space --space <spaceKey>` and compare its returned identity with the canonical file. This proves configured discovery and a service-account read, not a write or automatic dispatch.
...
4. **Automatic Stop dispatch:** complete a normal main-thread turn without manually invoking `codex-obs`. Verify that the trusted installed Stop hook ran, requested one continuation and produced exactly one observation pass. Manual success and installed source do not prove this step.
5. **Recursion suppression:** verify that the continued turn reaches Stop with `stop_hook_active=true` and that no second `codex-obs` dispatch occurs.
